<?php
/**
 * Step 2 of 2 — point the FluentCart updater at v2.7.3.
 *
...
$PREV_VER   = '2.7.2';
$PREV_FILE  = 256;
$KEY        = 'page-blocks-builder-v2.7.3.zip';
$BUCKET     = 'gauravtiwari-org-fluentcart';
$SIZE       = 166515;

$CHANGELOG = '<h4>2.7.3</h4><ul>'
	. '<li>Page settings — title, slug, template, import and export — now live together in one dialog instead of being spread across the builder toolbar and the section panel.</li>'
	. '<li>Sections get an id automatically. Where the outermost element has none the builder writes one in; an id you wrote yourself is never touched, and your formatting is preserved exactly.</li>'
	. '<li>Every section can be dragged, from anywhere on its row, including locked and linked ones. Previously only a 14px grip worked.</li>'
	. '<li>The AI assistant applies its own answers: a reply lands in the section and the preview updates, rather than arriving as code to copy across. Undo restores the section exactly, and a Whole page toggle sends the rest of the page as context so generated markup matches what is already there.</li>'
	. '<li>Latest AI models, with GPT-5.6 Luna as the default. The model list had drifted across four places, so a saved choice could be silently replaced by another.</li>'
	. '<li>Locked Core and Custom blocks can now be deleted from the section panel, and clicking anything in the preview selects the section it belongs to.</li>'
	. '<li>Fixed: on themes with a dark mode the builder was borrowing it — dropdowns, inputs and code blocks rendered dark inside light chrome, and the preview could show dark-on-dark text.</li>'
	. '<li>Fixed: Detach a copy rendered as an unstyled browser button. Clearer names in the code toolbar: wpauto is Auto-format, PHP is Run PHP.</li>'
	. '</ul>';
...
$ok   = ! is_wp_error( $resp ) && (int) wp_remote_retrieve_response_code( $resp ) === 200
	...
	&& hash_file( 'sha256', $tmp ) === '3bd82f81d8cebb2d7db23b9cff356a8b78a520e32d6e5bc5d3e41d14872d7eb5';
@unlink( $tmp );
if ( ! $ok ) {
