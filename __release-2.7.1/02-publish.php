...
$PREV_VER   = '2.6.0';
$PREV_FILE  = 93;
$KEY        = 'page-blocks-builder-v2.7.1.zip';
$BUCKET     = 'gauravtiwari-org-fluentcart';
$SIZE       = 138023;

$CHANGELOG = '<h4>2.7.1</h4><ul>'
	. '<li>Fixed: the code editors could not be clicked into or typed in inside the block canvas. WordPress iframes that canvas, and the code editor library loads into the outer admin document, so it rendered but never received focus or keystrokes.</li>'
	. '<li>Fixed: the block editor UI rendered unstyled, with blank squares in place of the device-preview controls.</li>'
	. '<li>Fixed: a block\'s CSS could be silently dropped when it was rendered from a widget, synced pattern or template part.</li>'
	. '<li>Fixed: CSS minification corrupted quoted selectors such as [style*="font-weight: 300"], and the required spaces inside calc() and clamp() math.</li>'
	. '<li>Previews now load theme.json global styles alongside every theme stylesheet, so var() resolves as it does on the front end.</li>'
	. '<li>The CSS editor suggests the custom properties your active theme defines.</li>'
	. '<li>The editor chrome now follows the WordPress admin\'s own palette, spacing and buttons.</li>'
	. '</ul>';
...
$ok   = ! is_wp_error( $resp ) && (int) wp_remote_retrieve_response_code( $resp ) === 200
	...
	&& hash_file( 'sha256', $tmp ) === '070810b83f83d0e5d1a55c71d030ab29eed7e46ad0f9b49175cf22c3392fb2f9';
@unlink( $tmp );
if ( ! $ok ) {
